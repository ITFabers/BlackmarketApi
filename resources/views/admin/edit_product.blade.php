                        ...
                        <form action="{{ route('admin.product.update',$product->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-12"  id="dynamicDropdowns">
                                    ...
                                    @foreach ($selectedCategories as $key => $category)
                                      @if(isset($category) && !empty($category) && !empty($category->getChilds()))
                                      <select name="subcategories[]" class="form-control  categoryDropdown mt-3 def-cats" data-level="{{$key}}">
                                      ...
                                      @else
                                        <select name="subcategories[]" class="form-control select2 categoryDropdown" data-level="0">
                                            ...
                                            @foreach ($rootCategories as $category)
                                                <option value="{{ $category->id }}" {{( isset($selectedCategoryIds) && in_array($category->id, $selectedCategoryIds)) ? 'selected' : '' }}>{{ $category->name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                @foreach($product->attributes as $attribute)
                                    <div class="variant-select-row mt-3">
                                        <label for="variant">Select Specification</label>
                                        <select name="variant[]" class="variant form-control mt-3">
                                            <option value="">Select a specification</option>
                                            @foreach($variants as $variant)
                                                <option value="{{ $variant->id }}"
                                                    {{ $variant->id == $attribute->variant_id ? 'selected' : '' }}>
                                                    {{ $variant->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="variant_item[]" class="variant-item form-control mt-3">
                                             @foreach($attribute->variant->variantItems as $variantItem)
                                                 <option value="{{ $variantItem->id }}"
                                                     {{ $variantItem->id == $attribute->variant_item_id ? 'selected' : '' }}>
                                                     {{ $variantItem->name }}
                                                 </option>
                                             @endforeach
                                         </select>
                                         <input type="text" class="form-control mt-3 variant-text" placeholder="Text" name="text[]" value="{{$attribute->text}}">
                          ...
                          <div class="form-group col-12">
                              ...
                              <select name="show_homepage" class="form-control">
                                  <option {{ $product->show_homepage == 0 ? 'selected' : '' }} value="0">{{__('admin.No')}}</option>
                                  <option {{ $product->show_homepage == 1 ? 'selected' : '' }} value="1">{{__('admin.Yes')}}</option>
                              </select>
                          </div>
                            <div class="row">
                                <div class="col-12">
